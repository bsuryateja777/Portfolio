...
export default function Experience() {
  ...
  return (
    <motion.div id="experience" className="w-full px-[12%] pt-10 scroll-mt-8 lg:scroll-mt-10 2xl:scroll-mt-14" initial={{ opacity: 0 }} whileInView={{ opacity: 1 }}  viewport={{ once: true }} transition={{ duration: 1, delay: 0.3 }}>
      ...
      <div className="relative mt-10">
        {/* Vertical line */}
        <motion.div className="absolute left-3.5 top-2 lg:top-4 2xl:top-3 bottom-10 lg:bottom-9 2xl:bottom-8 w-1 bg-blue-900" initial={{ opacity: 0, x: -10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}></motion.div>

        {sortedExperience.map((exp, index) => {
          const isCurrent = index === 0;

          return (
            <div key={exp.organisation} className="flex items-start mb-10 relative">
              {/* Dot container */}
              ...
              <motion.div className="ml-6 flex flex-col lg:flex-row lg:items-center lg:gap-4" initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.4 }}>
                <motion.div initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1 }}>
                  <h3 className="font-bold text-lg">{exp.organisation}</h3>
                  <p className="text-sm text-gray-700 dark:text-gray-300">{exp.position}</p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">{exp.duration}</p>
                </motion.div>
                {exp.description && (
                  <motion.p className="hidden lg:flex text-sm text-gray-600 dark:text-gray-400 lg:ml-4 lg:border-l dark:border-gray-600 lg:pl-4" initial={{ opacity: 0, x: 10 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1, delay: 0.7 }}>
                    {exp.description}
                  </motion.p>
                )}
              </motion.div>
